...
#include "bsp_st7789_font.h"

#if USE_HARDWARE_SPI == 1

#include "spi.h"

EXT_SRAM static uint8_t fill_buf[LCD_FILL_BUF_SIZE];
...
{
  uint32_t w = xend - xsta;
  uint32_t h = yend - ysta;
  uint32_t pixels = w * h;
  ...
  for (uint32_t i = 0; i < LCD_FILL_BUF_SIZE; i += 2)
  {
    fill_buf[i] = color_h;
  ...
  while (pixels > 0)
  {
    ...
    if (chunk_pixels > LCD_FILL_BUF_SIZE / 2)
    {
      chunk_pixels = LCD_FILL_BUF_SIZE / 2;
    }
    // HAL_SPI_Transmit(&hspi1, fill_buf, chunk_pixels * 2, HAL_MAX_DELAY);
    if (HAL_SPI_Transmit_DMA(&hspi1, fill_buf, chunk_pixels * 2) != HAL_OK)
    {
      Error_Handler();
    }
...
void LCD_WriteColorBuffer(u16 xsta, u16 ysta, u16 xend, u16 yend, const u16 *color)
{
  ...
  while (pixels > 0)
  {
    ...
    {
      u16 pixel = color[i];
      fill_buf[i * 2] = pixel >> 8;
      fill_buf[i * 2 + 1] = pixel & 0xFF;
    }
...
void LCD_Writ_Bus(u8 dat)
{
  ...
  #elif USE_HARDWARE_SPI == 1
  /* Command/data bytes must finish before DC changes or the next byte starts. */
  if (HAL_SPI_Transmit(&hspi1, &dat, 1, HAL_MAX_DELAY) != HAL_OK)
  {
    Error_Handler();
  }
  #endif
